...
### Модуль «Касса» (cash-desk-ui) — отдельный порт, без авторизации

Модуль **cash-desk-ui** — отдельное приложение на порту **8081** с формой пополнения и снятия, без входа в систему.

1. Сначала запустите основное приложение (порт 8080).
2. В другой консоли из корня проекта:
```bash
cd cash-desk-ui
mvn spring-boot:run
```
...
- **Пополнение (Top-up)** — отправляет `POST /api/payment/top-up` с полем `amount`.
- **Снятие (Cashout)** — отправляет `POST /api/cashout/request` с полем `amount`.

Авторизация не требуется. Запросы идут на API основного сервиса (http://localhost:8080). CORS настроен для источника http://localhost:8081.
